...
$(function() {
	...
	var gnb_v = 'N', gnb_s = 'N', gnb_w = 'N', fix_1 = 'N', fix_2 = 'N', fix_leave = 'N', fix_w = 'N', change_w = 'N', sh_g1_g2 = 'N', click_w = 'N', gnb_s1_cs = 'N', gnb_v_cnt = '', set_g1_nm = 'N';
	...
	if (gnb_wrap.is('.set-g1-nm')) set_g1_nm = 'Y';
	...
	if (gnb_w === 'Y') {
		...
		if (sh_g1_g2 === 'Y') keep_class += ' sh-g1-g2'; if (change_w === 'Y') keep_class += ' change-w'; if (set_g1_nm === 'Y') keep_class += ' set-g1-nm';
		...
		gnb_sub_1.each(function(index) {
			...
			if (change_w === 'Y' && set_g1_nm === 'Y') { var gnb_li_ttl = $('>a', gnb_li.eq(index)).text(); obj_clone.attr('data-before-text', gnb_li_ttl); }
		...
		if (gnb_w !== 'Y') {
			...
			if (parent_ul.is('.ABA-gnb')) {												// GNB에 마우스 올린 경우
				if (change_w === 'Y') {													// 서브1차가 1개씩 노출되는 설정
					...
					if (this_gnb_sub_1.hasClass('AB-none')) { wide_gnb_sub.hide_nb(); return false; }
		...
		if (gnb_w !== 'Y') {
			...
			if (parent_ul.is('.ABA-gnb')) {
				if (change_w === 'Y') {
					...
					parent_ul_gnb_sub.css('display', 'none');
					var this_gnb_sub_1 = parent_ul_gnb_sub.eq(gnb_a.parent().index());
					if (this_gnb_sub_1.hasClass('AB-none')) { wide_gnb_sub.css('display', 'none'); return false; }
