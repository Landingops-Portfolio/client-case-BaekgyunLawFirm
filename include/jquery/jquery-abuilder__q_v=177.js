...
$(function() {
	...
	var obj_atmtl = $('#ab-title-move-to-layout'); if (obj_atmtl.length > 0) { var atmtl_ttl_sel = obj_atmtl.attr('data-ttl-sel'); if (atmtl_ttl_sel === undefined) atmtl_ttl_sel = 'h3.AB-sj-h3'; var obj_atmtl_ttl_sel = $(atmtl_ttl_sel); if (obj_atmtl_ttl_sel.length > 0) { obj_atmtl_ttl_sel.attr('data-pv', obj_atmtl_ttl_sel.html()); obj_atmtl_ttl_sel.prepend(obj_atmtl.html() + ' '); obj_atmtl.remove(); }}
  
	// 로딩중표시기
	$('body').on('click', '.loading-img', function(e) { $('#loading_img').loading_img({'cobj':$(this)}); });		// 로딩중 표시기 노출 마크업 클릭인 경우
	$.fn.loading_img = function(vars) { if (vars.off === 'Y') { this.css('display', 'none'); $('body').css('cursor', 'default'); } else { if (vars.cobj.attr('data-overlay') === 'Y') this.addClass('overlay-on'); this.css('display', ''); clearInterval(as_interval); }}
	if ($('#loading_img').length <= 0 && typeof(content_theme) !== 'undefined') $('<div style="display: none;" id="loading_img" class="core-load-wrap overlay-on"><img src="/template/DESIGN_content/program/theme/' + content_theme + '/loading_front.gif" alt="loading..." /></div>').appendTo('body');
	
	$('body').on('click', '.instantMsgWrap .msgClose', function(e) { $(this).parents('div').eq(0).css('display', 'none'); });
	
	// tr을 복사하여 추가하는 버튼
	// +버튼에 data-clone-div="input항목들 감싸는 div id값,복사할 수 있는 최대 개수,유지할 항목명(예:MUL_com_bos_time_log_basic_etc_2[])" 
	$('body').on('click', '.AB-copy-tr,.AB-copy-div', function(e) {
		...
		objTP.append(objApd); ab_date_picker();	
	});
	...
	$('body').on('click', 'a[href*="tel:"]', function(e) {
		...
		var tail = $(this).attr('data-tail'); if (tail !== undefined) url += '&' + tail;
		$.get(url, {}, function(data) {//console.log(url, data);
			var scts = [];
	...
	$('body').on('keyup', '*[data-chk-length]', function() {
		var obj_this = $(this), rel = obj_this.attr('rel'); if (rel === undefined) return false;
		var chkln = obj_this.data('chk-length'); if (chkln !== undefined && chkln > obj_this.val().length) return false;
		$.get(obj_this.attr('rel') + '?_tsss_=' + _tsss_ + '&pcv=' + obj_this.val(), function(d) { obj_this.real_time_verify_msg({'msg':'코드를 정확히 입력하세요.', 'result':d, 'cn':'chk-key', 'msc':(_VM_ === 'p' ? 'admin-help-in' : 'admin-help-in')}); });
	});
	...
	*/
	...
	$('body').on('click focusout change keypress', '*[data-direct-exec]', function(e) {
		...
		var exec_pass = $(this).get_evt_pass(e);
		if (exec_pass === 'N') {
			var form = []; if ($(this).attr('data-wrap-id') !== undefined) form = $($(this).attr('data-wrap-id')); if (form.length === 0) form = $(this).parents('form'); if (form.length === 0) form = $('body');
			var objTarg = $('#' + $(this).attr('data-direct-exec'), form); if (objTarg.length <= 0) return false; var hdobj = $(this); if ($(this).attr('data-hdobj-name') !== undefined) hdobj = form.find('input[name=' + $(this).attr('data-hdobj-name') + '],select[name=' + $(this).attr('data-hdobj-name') + ']');
			var execFunc = objTarg.attr('onclick'), objName = $(this).attr('data-obj-name'), rIdx = $(this).attr('data-obj-idx'), chgVals = []; chgVals.push(objName + '=' + hdobj.val().replaceAll("\r\n", "\\n").replaceAll("\n", "\\n").replaceAll(',', '­、­'));
			var etcobj, etcobjs = [], obj_den = $(this).attr('data-etcobj-names');//console.log(objTarg, execFunc + "\n", obj_den, form, form.find('input,select,textarea'));
			if (obj_den !== undefined) {
				if (obj_den === '*') {
					form.find('input,select,textarea').each(function(idx) { var nm = $(this).attr('name'); nm = (nm.substring(0, 4) === 'MOD_' ? nm.substring(4) : nm); chgVals.push(nm + '=' + $(this).val().replaceAll("\r\n", "\\n").replaceAll("\n", "\\n").replaceAll(',', '­、­')); });
				} else {
					etcobjs = obj_den.split(',');
					etcobjs.forEach(function(v, k, array) { etcobj = form.find('input[name=' + v + '],select[name=' + v + ']'); if (etcobj.length > 0) chgVals.push((v.substring(0, 4) === 'MOD_' ? v.substring(4) : v) + '=' + etcobj.val().replaceAll("\r\n", "\\n").replaceAll("\n", "\\n").replaceAll(',', '­、­')); });
				}
			}
			var cmpslt = $(this).attr('data-cmpslt'); if (cmpslt === undefined) cmpslt = 'table,tr,ul,div:has(input[name="list_select[]"])'; if (rIdx === undefined) rIdx = $(this).parents(cmpslt).find('input[name="list_serial[]"]').val();//console.log(chgVals);
			execFunc = execFunc.replace(/(SYSTEM_modify_article\([\s]*[^,]+[\s]*,[\s]*[^,]+[\s]*,[\s]*'[^']+'[\s]*,[\s]*')([^']*)('.+)$/, '$1' + (rIdx !== undefined ? rIdx : '$2' ) + '$3').replace('%RESERV%', chgVals.join(','));//console.log(execFunc + "\n");// return false;	
			eval(execFunc);
		}
	});
	...
	// data-sid-bxn : 세팅될 필드명이 선택된 박스 이름, data-targ-block : 변경할 대상(year, month, date, week), data-only-block : 변경할 대상만 바꾸려면 Y 시작일 범위를 넓히려면 O 종료일 범위를 넓히려면 C 둘다 넓히려면 A를 설정, data-fix-date : 날짜를 오늘로 고정하려면 Y 아니면 N, data-week-start : 주간 범위시 시작 요일(sun/기, mon)
	$('body').on('change', '.date-auto-set', function(e) {
		...
		if ($(this).val() != '') {
			...
			if (tb === 'week') {	// week 처리
				var today = new Date();
				var dayOfWeek = today.getDay(), offset = parseInt(tv[0], 10), weekStartOn = $(this).data('week-start') || 'sun'; 					// 시작 요일 옵션 가져오기 (기본값 'sun')
				var dateOffset; if (weekStartOn === 'mon') dateOffset = (dayOfWeek === 0) ? 6 : dayOfWeek - 1; else dateOffset = dayOfWeek;	// 기본은 일요일 시작, 월요일로 지정된 경우만 월요일 시작
				var thisWeekStartDate = new Date(today.getFullYear(), today.getMonth(), today.getDate() - dateOffset), thisWeekEndDate = new Date(today.getFullYear(), today.getMonth(), today.getDate() - dateOffset + 6);		// 1. '이번 주'의 시작일과 종료일을 계산
				open_date = new Date(thisWeekStartDate); open_date.setDate(thisWeekStartDate.getDate() + (offset * 7)); close_date = new Date(thisWeekEndDate); close_date.setDate(thisWeekEndDate.getDate() + (offset * 7));	// 2. 오프셋을 적용하여 목표하는 주의 시작일과 종료일 계산
			} else {					// year, month, date 처리
				var set_open_year, set_close_year, set_open_month, set_close_month, set_open_date, set_close_date;
				if (tb === 'year') set_year += parseInt(tv[0]); else if (tb === 'month') set_month += parseInt(tv[0]); else if (tb === 'date') set_date += parseInt(tv[0]);
				if (tv[0] <= 0) {
					set_open_year = set_year;
					...
					set_open_year = (fd === 'Y' ? date.getFullYear() : set_year);
					set_open_month = (fd === 'Y' ? date.getMonth() : (tb === 'year' && (ob === 'C' || ob === 'A') ? 11 : set_month));
					set_open_date = (ob === 'O' || ob === 'A' ? 1 : (fd === 'Y' ? date.getDate() : set_date));
				...
				if (tv[1] != undefined) { tv[1] = parseInt(tv[1]); if (tb === 'year') set_close_year += tv[1]; else if (tb === 'month') set_close_month += tv[1];/*필요시주석해제후사용else if (tb === 'week') set_close_date += tv[1] * 7; else if (tb === 'date') set_close_date += tv[1];*/ }
	...
	$('body').on('click', '*[class*=AB_btn_dialog_]', function(e) {
		...
		if (load_url !== undefined) {
			...
			var title, url_tail = '', is_modal = true, refresh_force = 'Y', ifrm = 'N', rs = true, rm = 'N', tt = 'Y', co = 'N', dgb = false, mdlscrl = 'N';
			...
			if (size[12] !== undefined) mdlscrl = size[12];
			...
			obj_global_dialog.dialog({
				...
				open: function(e, ui) {
					if (mdlscrl === 'N') $('html').css('overflow-y', 'hidden');
				...
				close: function(e, ui) {
					if (mdlscrl === 'N') $('html').css('overflow-y', 'scroll');
	...
	$.fn.ab_toggle_1 = function(callback1, callback2) {
		...
		this_obj.on('click', function(e) {
			if (toggle_cookie !== 'show') {
				if (only_class === undefined) {
					var obj_html_editor = $('textarea[id^="oEdit-"]', toggle_obj);
					if (ei_show.length === 1) { toggle_obj.css('display', ei_show[0]); if (obj_html_editor.length > 0) ab_editor_refresh(obj_html_editor); }
					else { toggle_obj.show(ei_show[0], {direction : ei_show[1]}, parseInt(ei_show[2]), function() { if (obj_html_editor.length > 0) ab_editor_refresh(obj_html_editor); }); }
				} else {
					toggle_obj.removeClass(oc_info[0]);
				}
				set_cookie(toggle_id, 'show', toggle_hour); this_obj.removeClass('-hide').addClass('-show'); toggle_cookie = 'show';
			} else {
				if (only_class === undefined) {
					if (ei_hide.length === 1) toggle_obj.css('display', ei_hide[0]);
					else toggle_obj.hide(ei_hide[0], {direction : ei_hide[1]}, parseInt(ei_hide[2]));
				...
				set_cookie(toggle_id, 'hide', toggle_hour); this_obj.removeClass('-show').addClass('-hide'); toggle_cookie = 'hide';
			}
			if (toggle_cookie === 'show') {
				$('body').addClass(this_obj_id + '-show');
				$('body').removeClass(this_obj_id + '-hide');
	...
	$.fn.category_load_next_box = function(vars, callback1, callback2, callback3, callback_gno_data) {
		...
		function _get_new_option(obj_chg, callback1, callback2, callback_gno_data) {	// change 이벤트가 발생되면 다음단계의 선택상자를 세팅하는 내부 함수
			...
			if (next_depth <= ctg_end) {											// 종료 단계보다 작거나 같으면 상자 추가 시도
				...
				$.getJSON(get_url, function(data) {				//console.log(get_url, data);
	...
	$.fn.set_value_targ_box = function(vars, callback) {
		if (this.length <= 0) return false;
		var fcsv, form_parent, this_obj = this;	// 클릭할 라디오(또는 체크상자들)
		var join = vars.join === undefined ? ';' : (vars.join !== 'X' ? vars.join : '');
		var bracket = vars.bracket === undefined ? ';' :  (vars.bracket !== 'X' ? vars.bracket : '');
		if (vars.form_id == '') form_parent = this.parents('form'); else form_parent = $('#' + vars.form_id, vars.targ_doc);	// 목록을 연 버튼의 폼 (부모폼)
		var obj_fcsv = form_parent.find('*[name=' + vars.input_boxs[0] + ']');																// for check saved value
		if (obj_fcsv.length === 0) { form_parent.append('<input type="hidden" name="' + vars.input_boxs[0] + '">'); obj_fcsv = form_parent.find('*[name=' + vars.input_boxs[0] + ']') }	// 첫번째 입력상자 없으면 임의로 만듬
		var obj_type = obj_fcsv.attr('type');//console.log(obj_type);
		...
		if (this_obj.prop('tagName') !== 'SELECT') {
		...
		this_obj.on('change', function(e) {
			...
			var ts; if ($(this).prop('tagName') === 'SELECT') ts = $('option:selected', $(this)); else ts = $(this); _set_value(ts);// console.log(ts);
		});
	...
	$.load_urls = function(json_arr, doc, obj_this) {
		...
		$.each(json_arr, function(idx, li) {
			...
			if (li.mode === 'G') {
				...
				$.each(postd, function(k, v) { if (isJquerySelector(v)) { if ($(v).length > 0) postd[k] = $(v).val(); } });	// 선택자가 넘어온 경우 해당 선택자 객체의 값을 post 전송할 값으로 세팅
	...
	$.fn.set_dv_phone = function(vars) {
		var lf, lm, ll, lr;
		var value = vars.val === undefined ? this.val() : vars.val; value = value.replace(/[^0-9]/g, '');
		...
		if (len_full > lf) {
			...
			value = value.replace(new RegExp(pt_str.join('')), rp_str.join('-')); if (vars.val === undefined) this.val(value); else return value;
		} else {
			if (vars.val === undefined) this.val(value); else return value;
		}
	}
	...
	// 지정된 주소를 ajax호출 하여 마크업을 적용, 웹명함생성 입력폼의 직급을 불러올 때 처음 사용
	//onchange="$(this).set_markup_url({'url':'insiter.php?design_file=com_bos_webnamecard_db_apply_wf_chi.php&article_num={GV_article_num}&idx=', 'bias_sel':'#styler_position_wrap', 'set_sel':'', 'ins_pos':'replace', 'debug':'Y', 'callback':function() { /*callback code*/ }})"
	//$('body').set_markup_url({'debug':'N', 'callback1':function(vars, data, url, obj_this) { return data.replace(new RegExp(dfbn + '_list', 'g'), dfbn); }, 'url':sch_url + data + '&AJAX=Y', 'bias_sel':wrap_item_list, 'ins_pos':'replace', 'set_sel':''});
	$.fn.set_markup_url = function(vars) {
		var obj_this = this; $('#loading_img').loading_img({'cobj':obj_this});
		var url = vars.url + obj_this.val(); if (vars.debug === 'Y') { console.log(vars); }
		$.get(url, {}, function(data) {//console.log(url, data);
			var vbs = typeof(vars.bias_sel) === 'object' ? vars.bias_sel : $(vars.bias_sel);
			if (vars.callback1 !== undefined) { if (vars.debug === 'Y') { console.log(data); } data = vars.callback1(vars, data, url, obj_this); if (vars.debug === 'Y') { console.log(data); } }
			var markup = vars.set_sel === undefined || vars.set_sel == '' ? $(data) : $(vars.set_sel, $(data));	// 데이터 영역의 html 얻음
			if (vars.ins_pos === 'before') vbs.before(markup); else if (vars.ins_pos === 'after') vbs.after(markup); else if (vars.ins_pos === 'prepend') vbs.prepend(markup); else if (vars.ins_pos === 'append') vbs.append(markup);
			else if (vars.ins_pos === 'replace') { vbs.html(''); vbs.append(markup); }
			$('#loading_img').loading_img({'cobj':obj_this, 'off':'Y'}); if (vars.callback2 !== undefined) vars.callback2(vars.vars); if (vars.debug === 'Y') { console.log(data, markup); }
		});
	}
...
function ab_editor_refresh(ohe) {
	//ab_toggle_1show시크롬에서편집기가열리지않아서이부분주석처리하고iframe:visible을iframe으로변경함20250830byleeif (browser_type.indexOf('IE') >= 0) {
		ohe.each(function(idx) {
			var atf = $(this).next('iframe');
			if (atf.length > 0) {
...
function ab_date_picker() {
	...
	if (AB_datepicker.length > 0) {
		...
		AB_datepicker.each(function(index) {									// 모든 datepicker 만큼 반목
			var obj_this = $(this), date_saved_value = obj_this.val();	// 기본 값 저장
			var date_format = 'yy-mm-dd', time_div = ' ', time_unit = '3';
			if (obj_this.attr('time_div') != undefined) time_div = obj_this.attr('time_div');
			if (obj_this.attr('time_unit') != undefined) time_unit = obj_this.attr('time_unit');
			if (obj_this.attr('year_range') != undefined) year_range = obj_this.attr('year_range'); else year_range = 'c-10:c+10';
			if (obj_this.attr('date_format') != undefined) date_format = obj_this.attr('date_format');
			if (obj_this.hasClass("AB_datepicker_time")) {	// 입력용 추가클래스(AB_datepicker_time) 지정된 경우
				...
				if (date_saved_value != '') {
					...
					if (split_date[1] == undefined || split_date[1] == '') obj_this.datepicker('option', 'dateFormat' , date_format_full);
					else obj_this.datepicker('option', 'dateFormat' , date_format + time_div + split_date[1]);
				} else {
					obj_this.datepicker('option', 'dateFormat' , date_format_full);
				}
			} else obj_this.datepicker('option', 'dateFormat' , date_format);
			if (obj_this.hasClass('AB_datepicker_btn')) {		// 입력용 추가클래스(AB_datepicker_time) 지정된 경우
				obj_this.datepicker('option', 'showOn' , 'both');
				if (typeof(admin_theme) !== 'undefined') obj_this.datepicker('option', 'buttonImage' , './cooker/theme/' + admin_theme + '/images/cal.gif');
				obj_this.datepicker('option', 'buttonImageOnly' , 'true');
				obj_this.datepicker('option', 'buttonText' , 'calendar');
			}
			if (obj_this.attr('data-min-date') != undefined) date_format = obj_this.datepicker('option', 'minDate' , obj_this.attr('data-min-date'));
			if (obj_this.attr('data-max-date') != undefined) date_format = obj_this.datepicker('option', 'maxDate' , obj_this.attr('data-max-date')); obj_this.datepicker('option', 'yearRange' , year_range); obj_this.val(date_saved_value);
			obj_this.next('img.ui-datepicker-trigger').on('mousedown touchstart', function(e) { if ($('.ui-datepicker').is(':visible')) obj_this.val(''); else if (obj_this.val() == '') obj_this.val(obj_this.prop('defaultValue')); });
		});
...
function submit_direct_ajax(obj, frm, action, board, serial_num, target, chg_values, query_string, after_script, after_msg, frm_attr, flag, p_self, return_url, _isa_, _isap_, callback) {
	...
	if (chg_values !== undefined && chg_values != '') {
		...
		for (i=0; i<chg_vars.length; i++) {
			...
			if (chg_vars_item[0] != '') chg_names[chg_vars_item[0]] = chg_vars_item[1].replaceAll('­、­', ',');	// 변경할 항목의 이름과 값 배열 번수
		}
	}
	...
	$.post(action, form.serialize() + '&' + $.param(chg_names), function(data) {
		console.log(action, ' => ', form.serialize(), ' => ', $.param(chg_names), ' => ', ai.ajax_area, ' => ', after_script, ' => ', data);
		if (after_script !== 'X') {
			var move_url;
			var alert_msg = get_alert_msg_in_result(data);									// 경고 메시지 있으면 출력
			if (alert_msg != 'null') after_submit_cancel(alert_msg);
...
function isJquerySelector(str) {
  if (!str || typeof str !== 'string' || str.trim() === '') return false;	// 1. 유효하지 않은 인자값 필터링
  const trimmedStr = str.trim(); if (trimmedStr.charAt(0) === '<' && trimmedStr.charAt(trimmedStr.length - 1) === '>') return false;	// 2. HTML 생성 문자열 제외
  try { jQuery(str); } catch (e) { return false; } return true;				// 3. jQuery 엔진을 통해 직접 유효성 검사(jQuery 엔진이 에러를 발생시키면 유효하지 않은 선택자)
}