...
function print_content(cid) {
	var print_content = document.getElementById(cid).innerHTML, print_window = window.open('', 'Print Window', 'height=600,width=800');
	print_window.document.head.innerHTML = document.head.innerHTML; print_window.document.body.innerHTML = print_content; print_window.focus(); print_window.print(); print_window.close();
}
...
// 입력된 글자수를 체크하여 메시지를 띄워주는 함수
function chkTxarea(obj, len, gb, obj_print_len_box, len_min, msg) {
	var return_value = true;
	var ln = get_str_byte(obj);
	//obj.value = trim(obj.value);
	if (gb == 'D' && ln == 0) { alert(len+lang_core[23]+ ln + ' Byte)'); obj.focus(); obj.select(); return_value = false; }
	if (obj_print_len_box != '') obj_print_len_box.value = ln;
	if (len_min != '') {
		if (ln < len_min) {
			if (return_value === true) { if (msg != '' && msg != undefined) alert(msg); else alert(len_min + lang_core[6] + ln + ' Byte)'); }
			obj.focus(); return_value = false;
		}
	}
	if (ln > len) {
		if (return_value === true) { if (msg != '' && msg != undefined) alert(msg); else alert(len + lang_core[23] + ln + ' Byte)'); }
		obj.focus(); return_value = false;
	} else {
		if (obj_print_len_box != '') obj_print_len_box.value = ln;
		//return true;
...
function disabled_except_obj(form, except_head, excepts) {
	...
	for (i=0; i<cnt; i++) {
		...
		for (j=0; j<excepts.length; j++) {													// 제외목록 처리 (name 값이 넘어오지 않는 객체도 비활성화 제외, fieldset 태그등)
			if (frm_els[i].name == '' || frm_els[i].name == excepts[j] || (excepts[j].substring(excepts[j].length-1, excepts[j].length) === '_' && frm_els[i].name.substring(0, excepts[j].length) === excepts[j]) || (excepts[j].substring(0, 1) === '_' && frm_els[i].name.substring(frm_els[i].name.length-excepts[j].length, frm_els[i].name.length) === excepts[j])) {
				flag = 'N';
				break;
			}
		}
